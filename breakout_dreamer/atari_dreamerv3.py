...
class Atari:
    ...
    def __init__(
        ...
        # Create environment with gymnasium and wrap it
        env_name = f"ALE/{name}-v5"
        with self.LOCK:
            gymnasium_env = gym.make(
                env_name,
            ...
            # Wrap to maintain old gym API
            self._env = GymnasiumToGymWrapper(gymnasium_env)
        
        # Verify NOOP is the first action
    ...
    def step(self, action):
        ...
        for repeat in range(self._repeat):
            # Now using old gym API: (obs, reward, done, info)
